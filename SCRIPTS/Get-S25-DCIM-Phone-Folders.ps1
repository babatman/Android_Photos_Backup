...
$DestDirsound_recorder = [string]"$destpath\TELEFON_VOICE_RECORDINGS_ALL"
$DestDirDownload = [string]"$destpath\Download_S25"

$DestDirForWhatsApp = [string]"$destpath\WhatsAPP"
$DestDirForTello= [string]"$destpath\TelloVideo"

[System.IO.Directory]::CreateDirectory($DestDirForCallRecordings)
[System.IO.Directory]::CreateDirectory($DestDirForVoiceRecordings)

$DestDirForViber = [string]"$destpath\TELEFON_VIBER_ALL"
$Summary = [Hashtable]@{NewFilesCount=0; ExistingFilesCount=0}
$phoneName = "MI 8" #Phone name as it appears in This PC
$phoneName = "Galaxy S25 Ultra"
$firstRootPhoneName = "Espace de stockage interne partagé"
$firstRootPhoneName = "Stockage interne"
$destRoot = "A:\BACKUP_ANDROID\RootFolderMI8-29-09-2022"
...
$destDirPath = $destDirPath -replace "\w{3,}\:","µ"
 ...
 foreach ($item in $items)
  ...
  }while($oldvalue -ne $existingCount)
}
...
Copy-FromPhoneSource-ToBackup (Get-SubFolder $phoneRootDir "$firstRootPhoneName\MIUI\sound_recorder") $DestDirForVoiceRecordings
Copy-FromPhoneSource-ToBackup (Get-SubFolder $phoneRootDir "$firstRootPhoneName\MIUI\sound_recorder\call_rec") $DestDirForVoiceRecordings
Copy-FromPhoneSource-ToBackup (Get-SubFolder $phoneRootDir "$firstRootPhoneName\Download") $DestDirDownload
#Copy-FromPhoneSource-ToBackup (Get-SubFolder $phoneRootDir "$firstRootPhoneName\Download") $DestDirForTello
...
#cd C:\Program Files\Android SDK platform\platform-tools_r31.0.3-windows\platform-tools>adb devices
